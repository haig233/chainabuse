                ...
                try:
                    ...
                    report_data = await page.evaluate('''() => {
                            ...
                            labeledInfos.forEach((info, index) => {
                                ...
                                if (label && label.textContent.trim() === 'Submitted') {
                                    ...
                                    if (index === 0 && textContent) {
                                        submittedTime = textContent;  // "4 hours ago"
                                    } else if (index === 1) {
                                        const linkLabel = info.querySelector('.create-Link__label');
                                        if (linkLabel) {
                                            submittedBy = linkLabel.textContent.trim();  // "Anonymous"
                                        }
                                    }
                                }
                            });
                        ...
                        if (lossesSection) {
                            ...
                            if (lossPs[1]) {
                                lossAmount = lossPs[1].textContent.trim();  // "20000.00 USD"
                            }
                        }
...
async def scrape_batch(urls, max_concurrent=3, retry_count=0):
    """Process batch of URLs with progress bar - reduced concurrency for clicking"""
...
def save_batch_file(batch_num, url_start, url_end, urls_data, batch_dir):
    """Save batch of 50 URLs with all their data and push to GitHub"""
    ...
    git_commit_and_push(f"Add batch-{batch_num} (URLs {url_start}-{url_end})")
...
def save_checkpoint(all_reports, processed, failed_urls, total_urls, stats, permanently_failed=None):
    ...
    git_commit_and_push(f"Checkpoint at {processed} URLs")
...
async def scrape_all_github(all_urls, batch_size=50, max_concurrent=3, checkpoint_interval=250, max_retries=2):
    """Scrape with batch saving and GitHub push every 50 URLs"""
    all_reports = []
    failed_urls = []
    permanently_failed = []
    start_index = 0
    ...
    try:
        for i in range(0, len(urls_to_process), batch_size):
            ...
            try:
                ...
                # Save batch file and push to GitHub
                save_batch_file(
                    batch_num, 
                    current_index, 
...
async def main():
    ...
    try:
        all_reports, permanently_failed, stats, batch_dir = await scrape_all_github(
            all_urls=urls_list,
            batch_size=50,
            max_concurrent=2,  # Further reduced for stability
        ...
        git_commit_and_push(f"Complete scraping - {len(all_reports)} reports")
...
# Fix: main() is async, need to run it properly
